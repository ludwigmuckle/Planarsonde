...
class B2910BLDriver:
    ...
    def _apply_sweep_trigger(self, n_points: int, trigger_interval: float) -> None:
        ...
        self.inst.write(":TRIG:SOUR AUTO")                        # Internal auto-trigger (no BNC needed)
        self.inst.write(f":TRIG:DEL {trigger_interval:.6g}")      # Delay ≥ 50 µs between triggers
        self.inst.write(f":TRIG:COUN {n_points:d}")               # Fire exactly n_points triggers
    ...
    def _fetch_data(self, n_points: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        V = values[0::2][:n_points]   # Even indices → voltage
        I = values[1::2][:n_points]   # Odd  indices → current
        return V, I
